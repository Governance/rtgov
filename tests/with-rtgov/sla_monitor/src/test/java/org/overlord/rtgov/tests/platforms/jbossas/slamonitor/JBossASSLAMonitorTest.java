...
public class JBossASSLAMonitorTest {
    ...
    public static WebArchive createDeployment4() {
        ...
        java.io.File archiveFile=Maven.resolver().resolve("org.overlord.rtgov.samples.jbossas.sla:samples-jbossas-sla-monitor:war:"+version)
                .withoutTransitivity().asSingleFile();
        
        return ShrinkWrap.createFromZipFile(WebArchive.class,
                        TestUtils.copyToTmpFile(archiveFile,"slamonitor.war"));
    }
    ...
    public void testActivityEventsProcessed() {
        ...
        try {
            ...
            if (tl.getEvents(SITUATIONS_PROCESSED).size() != 6) {
                fail("Expecting 6 (sla situations) processed events, but got: "+tl.getEvents(SITUATIONS_PROCESSED).size());
            }
