 ...
 * interface, used to store situations in the SituationStore.
 *
 */
public class SituationStoreProcessor extends org.overlord.rtgov.ep.EventProcessor {
    ...
    public Serializable process(String source, Serializable event,
        ...
        if (event instanceof Situation) {
            SituationStore sitStore=SituationStoreFactory.getSituationStore();
            
            if (sitStore == null) {
                throw new Exception("Situation store has not been initialized");
            }
            
            sitStore.store((Situation)event);
        }
